...
const confirmDelBatch = () => {
  ...
  request.post('/volunteer/del/batch', idArr).then(res => {
    if (res.code === '200') {
...
// 导出接口
const exportData = () => {
  window.open(`http://${config.serverUrl}/volunteer/export`)
}
...
<template>
  <div>
    ...
    <div style="margin: 10px 0">
      <el-button type="success" @click="handleAdd" v-if="auths.includes('volunteer.add')">
        <el-icon style="vertical-align: middle">
          <Plus />
      ...
      <!--
      <el-upload
          v-if="auths.includes('volunteer.import')"
          class="ml5"
          :show-file-list="false"
          style="display: inline-block; position: relative; top: 3px"
          :action='`http://${config.serverUrl}/volunteer/import`'
      ...
      <el-button type="primary" @click="exportData" class="ml5" v-if="auths.includes('volunteer.export')">
        <el-icon style="vertical-align: middle">
          <Top />
        </el-icon>  <span style="vertical-align: middle"> 导出 </span>
      </el-button>
      -->
      <el-popconfirm title="您确定删除吗？" @confirm="confirmDelBatch" v-if="auths.includes('volunteer.deleteBatch')">
        <template #reference>
      ...
      <el-table-column prop="remark" label="备注"></el-table-column>
        ...
        <el-table-column label="操作" width="180">
          <template #default="scope">
            <el-button type="primary" @click="handleEdit(scope.row)" v-if="auths.includes('volunteer.edit')">编辑</el-button>
            <el-popconfirm title="您确定删除吗？" @confirm="del(scope.row.id)" v-if="auths.includes('volunteer.delete')">
              <template #reference>
    ...
    <el-dialog v-model="dialogFormVisible" title="义工信息信息" width="40%">
